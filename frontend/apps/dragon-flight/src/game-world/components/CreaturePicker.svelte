		...
		<div class="creature-grid">
			{#each CREATURES as creature (creature.id)}
				...
				>
					...
					<span class="copy"
						><strong>{creature.name}</strong><small>{creature.description}</small></span
					>
				</button>
			{/each}
		</div>
...
<style>
	...
	.creature-picker {
		box-sizing: border-box;
		display: flex;
		max-height: min(82dvh - 2px, 648px);
		flex-direction: column;
		padding: 17px;
		border: 4px solid #162a30;
		background: linear-gradient(145deg, rgba(25, 46, 51, 0.99), rgba(15, 31, 36, 0.99));
	}
	header {
		flex-shrink: 0;
		display: flex;
	...
	.creature-grid > button {
		...
		min-height: 64px;
		padding: 7px;
		border: 1px solid rgba(151, 111, 42, 0.52);
		background: rgba(2, 14, 9, 0.75);
		color: #c9b57d;
		text-align: left;
		cursor: pointer;
	}
	.creature-grid > button:hover,
	.creature-grid > button:focus-visible,
	.creature-grid > button.selected {
		border-color: #9dc5ad;
		color: #78efb6;
		outline: none;
		box-shadow: inset 0 0 16px rgba(37, 211, 133, 0.14);
	}
	.preview {
		position: relative;
		display: block;
	...
	.preview i::before {
		right: 55%;
		clip-path: polygon(100% 50%, 0 0, 18% 100%);
	}
	.preview i::after {
		left: 55%;
